...
I started with the Create React App and built off of it <a href="https://arthurdayton.com/post/1/" target="_blank">How to set up a Git Hub Pages Blog Site Using React and MDX</a>.  The approach for using S3 to serve the static site assets is essentially the same as using GitHub pages but when you start to think about putting an API on the back end I think it's "easier" to use AWS.
...
S3 (Simple Storage Service) is the OG of AWS.  It's cheap, easy to use, and easily integrates with lots of other AWS resources, such as Cloudfront.  It cost me around 3 cents last month so that seems very reasonable.
...
Cloudfront is a CDN which allows users to easily cache content around the world and as mentioned above easily integrates with S3 and Route53 so we can easily map our domain to our content.
...
My objective is to build a blog site that is deployed around the world and updates automatically whenever I push changes to the main branch of my repository.  I also want to create an API, using GraphQL, for getting and submitting comments for each blog entry.  I'll use a Lambda function to run GraphQL and DynamoDB as my data store so that my site is essentially free to deploy at low volume usage and will autoscale to any degree of usage.
...
This shows my hosted zone in AWS Console:
...
Using ACM(Amazon Certificate Manager) you can create a certificate for your domain and have it automatically validate by creating records in your hosted zone
</summary>

This shows the certificate status and domain validation status in ACM panel:
...
After creating a Cloudfront distribution for my S3 bucket, that I set as an origin, I can point the name records in Route53 at Cloudfront which I showed in Hosted Zone section above.  As with everything else we can automate and manage these with Terraform.
</summary>
...
This shows the configured S3 bucket origin where my React build artifacts are pushed from the Gitthub Actions pipeline that runs when I commit to my repository:
...
I reference my environment and then can pull secrets into my build job runs securely:
